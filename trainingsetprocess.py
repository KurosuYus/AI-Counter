...
def trainset_process(flag):
    ...
    if flag == 1:
        if os.path.isfile('fitness_poses_csvs_out/1.up.csv') and os.path.isfile('fitness_poses_csvs_out/1.down.csv'):
            return
    # 如果fitness_poses_csvs_out文件夹下的squat_down.csv和squat_up.csv已经存在，则不用导入样本图片再训练了
    elif flag == 2:
        if os.path.isfile('fitness_poses_csvs_out/2.up.csv') and os.path.isfile('fitness_poses_csvs_out/2.down.csv'):
            return
    # 如果fitness_poses_csvs_out文件夹下的pull_down.csv和pull_up.csv已经存在，则不用导入样本图片再训练了
    elif flag == 3:
        if os.path.isfile('fitness_poses_csvs_out/3.up.csv') and os.path.isfile('fitness_poses_csvs_out/3.down.csv'):
            return
        # 如果fitness_poses_csvs_out文件夹下的pull_down.csv和pull_up.csv已经存在，则不用导入样本图片再训练了
    elif flag == 4:
        if os.path.isfile('fitness_poses_csvs_out/4.up.csv') and os.path.isfile('fitness_poses_csvs_out/4.down.csv'):
            return
    # 指定样本图片的路径
